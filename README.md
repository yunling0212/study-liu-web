...
> ⚠️ **仓库归档说明（2026-09-20）**：本仓库为团队早期的技术验证骨架（FastAPI + 静态前端），已完成其架构验证使命。**比赛主仓库为 [Gitee `ai-coding`](https://gitee.com/zhang-binxiang666/ai-coding)**（Vue3 + Vite 前端，已部署上线），后续功能迭代均在那里进行。本仓库保留作为桌面版（tkinter，471 测试）到 Web 版的架构迁移记录，其中 FastAPI 路由设计、Docker 部署方案与 17 项冒烟测试仍具参考价值。
